...
class YOLOv3_base(chainer.Chain):
    """Implementation of YOLOv2(416*416).
    """
    def __init__(self, config, pretrained_model=None):
        super(YOLOv3_base, self).__init__()
        ...
        with self.init_scope():
            ...
            # Residual Block(1)
            self.conv3  = L.Convolution2D(64, 32, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn3    = L.BatchNormalization(32)
            self.conv4  = L.Convolution2D(32, 64, ksize=3, stride=1,
                                          pad=1, nobias=True)
            ...
            # Residual Block(2)
            self.conv6  = L.Convolution2D(128, 64, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn6    = L.BatchNormalization(64)
            self.conv7  = L.Convolution2D(64, 128, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn7    = L.BatchNormalization(128)
            self.conv8  = L.Convolution2D(128, 64, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn8    = L.BatchNormalization(64)
            self.conv9  = L.Convolution2D(64, 128, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn9    = L.BatchNormalization(128)

            # Down Sampling
            self.conv10 = L.Convolution2D(128, 256, ksize=3,
                                          stride=2, pad=1, nobias=True)
            self.bn10   = L.BatchNormalization(256)

            # Residual Block(8)
            self.conv11  = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn11    = L.BatchNormalization(128)
            self.conv12 = L.Convolution2D(128, 256, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn12   = L.BatchNormalization(256)
            self.conv13 = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn13   = L.BatchNormalization(128)
            self.conv14 = L.Convolution2D(128, 256, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn14   = L.BatchNormalization(256)
            self.conv15 = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn15   = L.BatchNormalization(128)
            self.conv16 = L.Convolution2D(128, 256, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn16   = L.BatchNormalization(256)
            self.conv17 = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn17   = L.BatchNormalization(128)
            self.conv18 = L.Convolution2D(128, 256, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn18   = L.BatchNormalization(256)
            self.conv19 = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn19   = L.BatchNormalization(128)
            self.conv20 = L.Convolution2D(128, 256, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn20   = L.BatchNormalization(256)
            self.conv21 = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn21   = L.BatchNormalization(128)
            self.conv22 = L.Convolution2D(128, 256, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn22   = L.BatchNormalization(256)
            self.conv23  = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn23    = L.BatchNormalization(128)
            self.conv24 = L.Convolution2D(128, 256, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn24   = L.BatchNormalization(256)
            self.conv25 = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn25   = L.BatchNormalization(128)
            self.conv26 = L.Convolution2D(128, 256, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn26   = L.BatchNormalization(256)

            # Down Sampling
            self.conv27 = L.Convolution2D(256, 512, ksize=3,
                                              stride=2, pad=1, nobias=True)
            self.bn27   = L.BatchNormalization(512)

            # Residual Block(8)
            self.conv28 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn28   = L.BatchNormalization(256)
            self.conv29 = L.Convolution2D(256, 512, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn29   = L.BatchNormalization(512)
            self.conv30 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn30   = L.BatchNormalization(256)
            self.conv31 = L.Convolution2D(256, 512, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn31   = L.BatchNormalization(512)
            self.conv32 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn32   = L.BatchNormalization(256)
            self.conv33 = L.Convolution2D(256, 512, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn33   = L.BatchNormalization(512)
            self.conv34 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn34   = L.BatchNormalization(256)
            self.conv35 = L.Convolution2D(256, 512, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn35   = L.BatchNormalization(512)
            self.conv36 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn36   = L.BatchNormalization(256)
            self.conv37 = L.Convolution2D(256, 512, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn37   = L.BatchNormalization(512)
            self.conv38 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn38   = L.BatchNormalization(256)
            self.conv39 = L.Convolution2D(256, 512, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn39   = L.BatchNormalization(512)
            self.conv40 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn40   = L.BatchNormalization(256)
            self.conv41 = L.Convolution2D(256, 512, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn41   = L.BatchNormalization(512)
            self.conv42 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn42   = L.BatchNormalization(256)
            self.conv43 = L.Convolution2D(256, 512, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn43   = L.BatchNormalization(512)

            # Down Sampling
            self.conv44 = L.Convolution2D(512, 1024, ksize=3,
                                          stride=2, pad=1, nobias=True)
            self.bn44   = L.BatchNormalization(1024)

            # Residual Block(4)
            self.conv45 = L.Convolution2D(1024, 512, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn45   = L.BatchNormalization(512)
            self.conv46 = L.Convolution2D(512, 1024, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn46   = L.BatchNormalization(1024)
            self.conv47 = L.Convolution2D(1024, 512, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn47   = L.BatchNormalization(512)
            self.conv48 = L.Convolution2D(512, 1024, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn48   = L.BatchNormalization(1024)
            self.conv49 = L.Convolution2D(1024, 512, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn49   = L.BatchNormalization(512)
            self.conv50 = L.Convolution2D(512, 1024, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn50   = L.BatchNormalization(1024)
            self.conv51 = L.Convolution2D(1024, 512, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn51   = L.BatchNormalization(512)
            self.conv52 = L.Convolution2D(512, 1024, ksize=3, stride=1,
                                          pad=1, nobias=True)
            self.bn52   = L.BatchNormalization(1024)

            ##################################################################
            self.conv53 = L.Convolution2D(1024, 512, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn53   = L.BatchNormalization(512)
            self.conv54 = L.Convolution2D(512, 1024, ksize=3,
                                          stride=1, pad=1, nobias=True)
            self.bn54   = L.BatchNormalization(1024)
            self.conv55 = L.Convolution2D(1024, 512, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn55   = L.BatchNormalization(512)
            self.conv56 = L.Convolution2D(512, 1024, ksize=3,
                                          stride=1, pad=1, nobias=True)
            self.bn56   = L.BatchNormalization(1024)
            self.conv57 = L.Convolution2D(1024, 512, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn57   = L.BatchNormalization(512)
            self.conv58 = L.Convolution2D(512, 1024, ksize=3,
                                          stride=1, pad=1, nobias=True)
            self.bn58   = L.BatchNormalization(1024)
            self.conv59 = L.Convolution2D(1024, out_ch, ksize=1,
                                          stride=1, pad=0, nobias=False)

            #######################conv57#####################################
            self.conv60 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn60   = L.BatchNormalization(256)
            # Upsample unpooling_2d
            # Concat bn43 , unpooling_2d 512 + 256
            ##################################################################
            self.conv61 = L.Convolution2D(768, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn61   = L.BatchNormalization(256)
            self.conv62 = L.Convolution2D(256, 512, ksize=3,
                                          stride=1, pad=1, nobias=True)
            self.bn62   = L.BatchNormalization(512)
            self.conv63 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn63   = L.BatchNormalization(256)
            self.conv64 = L.Convolution2D(256, 512, ksize=3,
                                          stride=1, pad=1, nobias=True)
            self.bn64   = L.BatchNormalization(512)
            self.conv65 = L.Convolution2D(512, 256, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn65   = L.BatchNormalization(256)
            self.conv66 = L.Convolution2D(256, 512, ksize=3,
                                          stride=1, pad=1, nobias=True)
            self.bn66   = L.BatchNormalization(512)
            self.conv67 = L.Convolution2D(512, out_ch, ksize=1,
                                          stride=1, pad=0, nobias=False)
            #######################conv65#####################################
            self.conv68 = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn68   = L.BatchNormalization(128)
            # Upsample unpooling_2d
            # Concat bn27 , unpooling_2d 256 + 128
            ##################################################################
            self.conv69 = L.Convolution2D(384, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn69   = L.BatchNormalization(128)
            self.conv70 = L.Convolution2D(128, 256, ksize=3,
                                          stride=1, pad=1, nobias=True)
            self.bn70   = L.BatchNormalization(256)
            self.conv71 = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn71   = L.BatchNormalization(128)
            self.conv72 = L.Convolution2D(128, 256, ksize=3,
                                          stride=1, pad=1, nobias=True)
            self.bn72   = L.BatchNormalization(256)
            self.conv73 = L.Convolution2D(256, 128, ksize=1,
                                          stride=1, pad=0, nobias=True)
            self.bn73   = L.BatchNormalization(128)
            self.conv74 = L.Convolution2D(128, 256, ksize=3,
                                          stride=1, pad=1, nobias=True)
            self.bn74   = L.BatchNormalization(256)
            self.conv75 = L.Convolution2D(256, out_ch, ksize=1,
                                          stride=1, pad=0, nobias=False)
    ...
    def model(self, x):
        ...
        # Residual (1)
        h1 = F.leaky_relu(self.bn3(self.conv3(h)), slope=0.1)
        h += F.leaky_relu(self.bn4(self.conv4(h1)), slope=0.1)
        # Down
        h = F.leaky_relu(self.bn5(self.conv5(h)), slope=0.1)
        # Residual (2)
        h1 = F.leaky_relu(self.bn6(self.conv6(h)), slope=0.1)
        h += F.leaky_relu(self.bn7(self.conv7(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn8(self.conv8(h)), slope=0.1)
        h += F.leaky_relu(self.bn9(self.conv9(h1)), slope=0.1)
        # Down
        h = F.leaky_relu(self.bn10(self.conv10(h)), slope=0.1)
        # Residual (8)
        h1 = F.leaky_relu(self.bn11(self.conv11(h)), slope=0.1)
        h += F.leaky_relu(self.bn12(self.conv12(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn13(self.conv13(h)), slope=0.1)
        h += F.leaky_relu(self.bn14(self.conv14(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn15(self.conv15(h)), slope=0.1)
        h += F.leaky_relu(self.bn16(self.conv16(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn17(self.conv17(h)), slope=0.1)
        h += F.leaky_relu(self.bn18(self.conv18(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn19(self.conv19(h)), slope=0.1)
        h += F.leaky_relu(self.bn20(self.conv20(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn21(self.conv21(h)), slope=0.1)
        h += F.leaky_relu(self.bn22(self.conv22(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn23(self.conv23(h)), slope=0.1)
        h += F.leaky_relu(self.bn24(self.conv24(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn25(self.conv25(h)), slope=0.1)
        h += F.leaky_relu(self.bn26(self.conv26(h1)), slope=0.1)
        down_8 = h
        # Down
        h = F.leaky_relu(self.bn27(self.conv27(h)), slope=0.1)
        # Residual (8)
        h1 = F.leaky_relu(self.bn28(self.conv28(h)), slope=0.1)
        h += F.leaky_relu(self.bn29(self.conv29(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn30(self.conv30(h)), slope=0.1)
        h += F.leaky_relu(self.bn31(self.conv31(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn32(self.conv32(h)), slope=0.1)
        h += F.leaky_relu(self.bn33(self.conv33(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn34(self.conv34(h)), slope=0.1)
        h += F.leaky_relu(self.bn35(self.conv35(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn36(self.conv36(h)), slope=0.1)
        h += F.leaky_relu(self.bn37(self.conv37(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn38(self.conv38(h)), slope=0.1)
        h += F.leaky_relu(self.bn39(self.conv39(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn40(self.conv40(h)), slope=0.1)
        h += F.leaky_relu(self.bn41(self.conv41(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn42(self.conv42(h)), slope=0.1)
        h += F.leaky_relu(self.bn43(self.conv43(h1)), slope=0.1)
        down_16 = h
        # Down
        h = F.leaky_relu(self.bn44(self.conv44(h)), slope=0.1)
        # Residual (8)
        h1 = F.leaky_relu(self.bn45(self.conv45(h)), slope=0.1)
        h += F.leaky_relu(self.bn46(self.conv46(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn47(self.conv47(h)), slope=0.1)
        h += F.leaky_relu(self.bn48(self.conv48(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn49(self.conv49(h)), slope=0.1)
        h += F.leaky_relu(self.bn50(self.conv50(h1)), slope=0.1)
        h1 = F.leaky_relu(self.bn51(self.conv51(h)), slope=0.1)
        h += F.leaky_relu(self.bn52(self.conv52(h1)), slope=0.1)
        h = F.leaky_relu(self.bn53(self.conv53(h)), slope=0.1)
        h = F.leaky_relu(self.bn54(self.conv54(h)), slope=0.1)
        h = F.leaky_relu(self.bn55(self.conv55(h)), slope=0.1)
        h = F.leaky_relu(self.bn56(self.conv56(h)), slope=0.1)
        h = F.leaky_relu(self.bn57(self.conv57(h)), slope=0.1)
        h1 = F.leaky_relu(self.bn58(self.conv58(h)), slope=0.1)
        out_32 = self.conv59(h1)
        # B, C, H, W = out_32.shape
        # out_32 = F.transpose(out_32, (0, 2, 3, 1))
        # out_32.= = out_32.reshape(B, C, -1)

        # Upsample
        h = F.leaky_relu(self.bn60(self.conv60(h)), slope=0.1)
        h = F.concat([F.unpooling_2d(h, 2, cover_all=False), down_16], axis=1)
        h = F.leaky_relu(self.bn61(self.conv61(h)), slope=0.1)
        h = F.leaky_relu(self.bn62(self.conv62(h)), slope=0.1)
        h = F.leaky_relu(self.bn63(self.conv63(h)), slope=0.1)
        h = F.leaky_relu(self.bn64(self.conv64(h)), slope=0.1)
        h = F.leaky_relu(self.bn65(self.conv65(h)), slope=0.1)
        h1 = F.leaky_relu(self.bn66(self.conv66(h)), slope=0.1)
        out_16 = self.conv67(h1)

        # Upsample
        h = F.leaky_relu(self.bn68(self.conv68(h)), slope=0.1)
        h = F.concat([F.unpooling_2d(h, 2, cover_all=False), down_8], axis=1)
        h = F.leaky_relu(self.bn69(self.conv69(h)), slope=0.1)
        h = F.leaky_relu(self.bn70(self.conv70(h)), slope=0.1)
        h = F.leaky_relu(self.bn71(self.conv71(h)), slope=0.1)
        h = F.leaky_relu(self.bn72(self.conv72(h)), slope=0.1)
    ...
    def inference(self, imgs, img_shape):
        ...
        with chainer.using_config('train', False), \
            ...
            for i, output in enumerate(output_list):
                ...
                prob = F.sigmoid(prob).data
                bbox_list.append(bbox_pred.reshape(N, -1, 4))
                conf_list.append(conf.reshape(N, -1))
                prob_list.append(prob.reshape(N, -1, self.n_classes))
            bbox_list = self.xp.concatenate(bbox_list, axis=1)
            conf_list = self.xp.concatenate(conf_list, axis=1)
