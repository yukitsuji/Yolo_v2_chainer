...
from utils.cython_util.nms import nms_gt_anchor, nms_gt_anchor_v3
...
class Transform(object):
    ...
    def __init__(self, value, dim=None, max_target=30, batchsize=1,
        ...
        self.downscale = np.array(downscale, dtype='i')
        self.i = 0
        self.count = 0
...
def create_map_anchor_gt(bbox, anchors, output_shape, downscale, n_boxes,
    ...
    """
    Args:
        bbox(array): Shape is (Boxes, 4). (y, x, y, x). scale=[0, 1].

    Returns:
        gmap(array): Shape is (max_target, 3). (x, y, box) index.
    """
